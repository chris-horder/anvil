...
Storage = (function() {
  ...
  Storage.prototype.get = function(filename, cb) {
    s3 = new AWS.S3();
    params = {Bucket: process.env.S3_BUCKET, Key: CleanName(filename)};
    a = s3.getObject(params )
    return cb(null, a);
  };
  ...
  Storage.prototype.create = function(filename, data, options, cb) {
    ...
    params = {Bucket: process.env.S3_BUCKET, ACL: "private", Key: CleanName(filename), Body: data,ContentType : options["Content-Type"], ContentLength : options["Content-Length"]};
  ...
  Storage.prototype.create_stream = function(filename, stream, cb) {
    ...
    params = {Bucket: process.env.S3_BUCKET, ACL: "private", Key: CleanName(filename), Body: stream};
    s3.putObject(params, function(err, data) {
      return cb(data);
    });
